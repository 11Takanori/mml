...
let pp_val = function
  | IntV i -> print_int i
  | BoolV b -> print_string (string_of_bool b)
  | ProcV (_, _, _) -> print_string "<fun>"
...
let rec eval_exp env = function
  ...
  | LetRecExp (id, param, exp1, exp2) ->
      let dummyenv = ref Environment.empty in
      let newenv = Environment.extend id (ProcV (param, exp1, dummyenv)) env in
      dummyenv := newenv ;
